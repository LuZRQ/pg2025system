                    ...
                    <div id="catalogo" class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-3 gap-4 sm:gap-6">
                        ...
                        @forelse ($productos as $producto)
                            <div class="producto-card bg-white rounded-2xl shadow-md hover:shadow-xl overflow-hidden border border-amber-200 transform hover:scale-[1.02] transition-all duration-200 ease-in-out"
                                ...
                                {{-- Imagen --}}
                                <div
                                    class="h-28 sm:h-32 bg-gradient-to-tr from-amber-200 to-amber-400 flex items-center justify-center overflow-hidden">
                                ...
                                <div class="p-4">
                                    ...
                                    @if ($producto->variantes && $producto->variantes->count() > 0)
                                        <select
                                            class="select-variante w-full mb-2 border rounded-lg px-2 py-1 text-sm text-amber-800"
                                    ...
                                    <button
                                        ...
                                        data-precio="{{ $producto->precio }}" data-stock="{{ $producto->stock_inicial }}"
                                        {{-- stock del producto --}}
                                        @if ($producto->variantes && $producto->variantes->count() > 0) data-tiene-variantes="true"
       data-variantes="{{ json_encode(
        ...
        @if ($rol === 'Mesero')
            <div class="mt-6 mb-10">
                ...
                @php
                    ...
                    $pedidosHoy = $pedidosActuales->filter(
                        fn($p) => \Carbon\Carbon::parse($p->fechaCreacion)->isToday(),
                    );
                @endphp
                            ...
                            <div
                                ...
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <h3 class="text-lg font-semibold {{ $text }}">
                                        Pedido N° {{ $pedido->numero_diario ?? $pedido->idPedido }} - Mesa
                                        {{ $pedido->mesa }}
                                    </h3>
...
=========================== --}}
        @php
            ...
            $pedidosListosHoy = $pedidos->filter(
                fn($p) => \Carbon\Carbon::parse($p->fechaCreacion)->isToday() &&
                    ($p->estado == 'listo' ||
                        $p->detalles->where('estado', 'listo')->count() > 0 ||
                        $p->detalles->where('es_nuevo', 1)->count() > 0),
            );
        @endphp
                ...
                <div id="pedidos-listos" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach ($pedidosListosHoy as $pedido)
                        <div class="border rounded-lg p-4 shadow-sm">
                            ...
                            {{-- Productos listos (antiguos o nuevos ya procesados) --}}
                            @php
                                $listos = $pedido->detalles->where('estado', 'listo');
                                $pendientes = $pedido->detalles->where('estado', 'pendiente')->where('es_nuevo', 1);
                            @endphp

                            @if ($listos->count() > 0)
                                <h4 class="text-sm text-green-700 font-semibold">Listos:</h4>
                                <ul class="text-sm text-green-800 mb-3">
                                    @foreach ($listos as $detalle)
                                        <li>- {{ $detalle->cantidad }} x {{ $detalle->producto->nombre }}
                                            @if ($detalle->variante)
                                                ({{ $detalle->variante->tipo }})
                                            @endif
                                        </li>
                                    @endforeach
                                </ul>
                            ...
                            {{-- BOTÓN ABRIR MODAL --}}
                            <button class="bg-amber-600 hover:bg-amber-700 text-white px-3 py-2 rounded-lg w-full mt-2"
...
=========================== --}}
<div id="modal-agregar-productos" class="hidden fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
    ...
    <div class="bg-white w-full max-w-5xl rounded-xl shadow-lg p-6 border border-amber-300">
        ...
        <form id="form-agregar-productos" method="POST" action="">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {{-- ======================
                     CATALOGO PRODUCTOS DEL MODAL
                ====================== --}}
                <div>
                    <label class="font-semibold text-amber-800">Categoría</label>
                    <select id="modal-select-categoria" class="w-full mb-3 border rounded-lg px-2 py-1">
                        <option value="all">Todas</option>
                   ...
                   <div id="modal-lista-productos" class="space-y-2 max-h-[420px] overflow-y-auto">

    @foreach ($productosModal as $producto)  {{-- ✅ USAMOS TODOS LOS PRODUCTOS --}}
    ...
    @endforeach
                ...
                <div>
                    ...
                    <div class="mb-4 mt-2">
                        <label class="block text-sm text-amber-700">Comentarios</label>
                        <textarea name="comentario" id="modal-comentario-text" class="w-full border rounded-lg p-2 mt-1 text-sm"
                                  placeholder="Ej: sin picante, poca sal..."></textarea>
                    </div>
...
=========================== --}}
        <script>
            //////// ESTO ES DEL PEDIDO PRINCIPAL no tocar////
   ...
   // MODAL AGREGAR PRODUCTOS
    ...
    // Filtrar categorías solo dentro del modal
    document.getElementById("modal-select-categoria").addEventListener("change", function() {
    ...
    // Agregar productos a la lista temporal
    document.querySelectorAll('.btn-add-modal').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (idx !== -1) {
                ...
                nuevosProductos.push({ idProducto: id, nombre, precio, cantidad, variante_id, variante_tipo });
            }
    ...
    function renderListaNuevos() {
        ...
        form.onsubmit = () => {
            ...
            const comentario = document.getElementById('modal-comentario-text').value;

    ...
    <div class="bg-white rounded-2xl shadow-md p-4 mb-6 border border-amber-200">
        <form method="GET" action="{{ route('ventas.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                ...
                <input type="date" name="fecha_desde" class="w-full mt-1 rounded-lg border-gray-300 shadow-sm focus:ring-amber-400 focus:border-amber-400">
            </div>
            <div>
                <label class="text-sm font-semibold text-amber-700">Fecha hasta</label>
                <input type="date" name="fecha_hasta" class="w-full mt-1 rounded-lg border-gray-300 shadow-sm focus:ring-amber-400 focus:border-amber-400">
            </div>
            <div>
                <label class="text-sm font-semibold text-amber-700">Cliente</label>
                <input type="text" name="cliente" placeholder="Nombre cliente" class="w-full mt-1 rounded-lg border-gray-300 shadow-sm focus:ring-amber-400 focus:border-amber-400">
            </div>
            <div class="flex items-end">
                <button type="submit" class="w-full bg-amber-600 hover:bg-amber-700 text-white py-2 rounded-lg shadow-md transition">
                    Filtrar
                </button>
            </div>
        </form>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-amber-50 transition">
                    ...
                    <td class="px-4 py-3 text-center space-x-2">
                        <!-- Ver -->
                        <a href="{{ route('ventas.show', $venta->id) }}" class="inline-flex items-center px-2 py-1 text-sm text-blue-600 hover:text-blue-800">
                            <i class="fas fa-eye"></i>
                        </a>
                        <!-- Editar -->
                        <a href="{{ route('ventas.edit', $venta->id) }}" class="inline-flex items-center px-2 py-1 text-sm text-amber-600 hover:text-amber-800">
                        ...
                        <!-- Descargar Factura -->
                        <a href="{{ route('ventas.factura', $venta->id) }}" class="inline-flex items-center px-2 py-1 text-sm text-green-600 hover:text-green-800">

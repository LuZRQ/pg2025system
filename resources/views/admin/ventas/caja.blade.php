    ...
    <div class="grid grid-cols-12 gap-6">
        ...
        <div class="col-span-12 md:col-span-5 lg:col-span-4 p-2 md:p-4">
            <div class="bg-gradient-to-b from-yellow-100 to-yellow-200 rounded-2xl shadow-lg p-4 md:p-6 border border-amber-200">
                <h2 class="font-semibold text-lg md:text-xl text-amber-900 mb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                    <div class="bg-amber-100 rounded-lg p-3 shadow flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-base sm:text-lg text-amber-900" id="fondoInicial">Bs. {{ number_format($fondoInicial,2) }}</p>
                        </div>
                    </div>
                    <div class="bg-green-100 rounded-lg p-3 shadow flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-base sm:text-lg text-green-900" id="totalEnCaja">Bs. {{ number_format($totalEnCaja,2) }}</p>
                        </div>
                    </div>
                </div>

                {{-- Resumen de ventas --}}
                <h3 class="font-semibold text-md sm:text-lg text-amber-900 mb-2">
                    <i class="fas fa-chart-simple mr-1"></i>Resumen de Ventas
                </h3>
                <ul class="space-y-2 mb-4">
                    <li class="flex justify-between items-center">
                        <span class="flex items-center gap-1 text-amber-800"><i class="fas fa-money-bill-wave"></i> Efectivo</span>
                        <span class="font-semibold text-amber-900" id="totalEfectivo">Bs. {{ number_format($totalEfectivo,2) }}</span>
                    </li>
                    <li class="flex justify-between items-center">
                        <span class="flex items-center gap-1 text-amber-800"><i class="fas fa-credit-card"></i> Tarjeta</span>
                        <span class="font-semibold text-amber-900" id="totalTarjeta">Bs. {{ number_format($totalTarjeta,2) }}</span>
                    </li>
                    <li class="flex justify-between items-center">
                        <span class="flex items-center gap-1 text-amber-800"><i class="fas fa-qrcode"></i> QR</span>
                        <span class="font-semibold text-amber-900" id="totalQR">Bs. {{ number_format($totalQR,2) }}</span>
                    </li>
                </ul>
                ...
                <div class="flex flex-col gap-2">
                    <form action="{{ route('caja.cerrar') }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full bg-red-600 text-white py-2 rounded-md hover:bg-red-700 shadow-sm text-sm">
                            <i class="fas fa-lock mr-1"></i> Cerrar Caja
                        </button>
                    </form>

                    <div class="flex gap-2">
                        <a href="{{ route('caja.export.excel') }}" class="flex-1 bg-emerald-600 text-white py-2 rounded-md hover:bg-emerald-700 shadow-sm text-center text-sm">
                            <i class="fas fa-file-excel mr-1"></i> Excel
                        </a>
                        <a href="{{ route('caja.export.pdf') }}" class="flex-1 bg-indigo-600 text-white py-2 rounded-md hover:bg-indigo-700 shadow-sm text-center text-sm">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-4 md:p-6 border border-amber-200">
                ...
                <div class="flex flex-col md:flex-row justify-between mt-4 gap-2">
                    <a href="{{ route('ventas.caja') }}"
                       class="px-6 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 shadow text-center">
                        ✖ Cerrar
                    </a>

                    <form action="{{ route('ventas.cobrar') }}" method="POST" class="flex-1 md:flex-none">
                        @csrf
                        ...
                        <button type="submit"
                                class="w-full px-6 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 shadow">
                            ✔ Terminar orden
                        </button>
                    </form>
                </div>

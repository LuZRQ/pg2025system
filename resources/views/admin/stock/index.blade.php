    ...
    @foreach ($productos as $producto)
        ...
        <div class="bg-white p-4 rounded-lg shadow hover:shadow-md transition">
            ...
            @if ($producto->variantes->count() > 0)
                <div class="mt-2">
                    <span class="font-semibold text-gray-700 text-sm">Variantes:</span>
                    <ul class="text-xs list-disc list-inside mt-1 space-y-1">
                        @foreach ($producto->variantes as $variante)
                            <li>
                                {{ ucfirst($variante->tipo) }} — Stock: {{ $variante->stock }} — Precio: Bs. {{ number_format($variante->precio, 2) }}
                            </li>
                        @endforeach
                    </ul>
                </div>
    ...
    <table class="w-full text-left border-collapse">
        <thead class="bg-stone-100 text-stone-700 text-sm uppercase">
            <tr>
                ...
                <th class="px-4 py-2">Variante / Stock</th>
